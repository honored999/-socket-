...
static Loca shuju[1000];
static Loca sd_msg[1000];
static Loca_d shuju_d[1000];
static int i = 0;
static IpInfo* msg ;
...
void algorithm(int signum)
{
	...
	while (1) /*接收数据直至为空*/
	{
		...
		printf("chuli: %ld\n",timein.tv_sec*1000000 + timein.tv_usec);
		pthread_rwlock_wrlock(&rwlock);
		shuju[i] = recv_msg(msg,0);
		pthread_rwlock_unlock(&rwlock);
		
		shuju_d[i].data = shuju[i];
		shuju_d[i].AoI = timein.tv_sec*1000000 + timein.tv_usec - atol(shuju[i].time);
		shuju_d[i].label=1;
